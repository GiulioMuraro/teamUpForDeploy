<template>
    <div class="custom">
        <div class="ch max-w-4xl mx-auto p-6 border-4 border-green-500 rounded-lg">
            ...
            <div class="pb-8 mb-8 border-2 border-green-500 rounded-lg">
                ...
                <div>
                    <div v-for="prenotazione in prenotazioniUtente" :key="prenotazione._id" class="border-2 border-green-500 rounded-lg text-gray-800 py-2 px-4 mb-2" @click="selectPrenotazione(prenotazione)">
            ...
            <div class="border-2 border-green-500 rounded-lg pb-8 mb-8">
                <h2 id="fieldToReport" class="text-lg font-semibold mb-4">Campo da segnalare: {{ campoDaSegnalare.nome }}</h2>
            </div>
...
export default defineComponent({
    ...
    methods: {
        async fetchPrenotazioni(){
            ...
            try {
                const res = await fetch(`${config.BASE_URL}/prenotazioni`, {
                    method: 'POST',
                    headers: {
                ...
                this.prenotazioni = data.findPrenotazioni; // Imposta i dati dei campi disponibili nella proprietà dati del componente

                if(!data.success){
                this.error.status = true
                this.error.message = "Nessuna prenotazione trovata"
                }
            } catch (error) {
                console.error(error);
            }
